...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 py-4 md:py-6 px-6 md:px-12 flex justify-between items-center mix-blend-difference text-white">
      ...
      <div
        className={`fixed inset-0 bg-[#121212]/95 backdrop-blur-xl flex flex-col items-center justify-center gap-8 transition-all duration-500 md:hidden ${
          menuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
        }`}
        style={{ zIndex: 55, mixBlendMode: "normal" }}
      >
        {NAV_LINKS.map((link) => (
          <a
            key={link.href}
            ...
            className="text-2xl font-bold tracking-tight text-white/80 hover:text-white transition-colors uppercase"
          >
            {link.label}
          </a>
        ))}
        <a
          ...
          rel="noreferrer"
          onClick={() => setMenuOpen(false)}
          className="mt-4 px-8 py-3 rounded-full border border-white/20 text-sm font-semibold tracking-widest uppercase hover:bg-white hover:text-black transition-all"
        >
          Resume
        </a>
      </div>
